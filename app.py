...
# Version 1.6 (FishyNW palette, solid sidebar, improved header + sidebar contrast, ASCII-safe)
...
:root {
  ...
  --menu_text: rgba(248,248,232,0.92);
  --menu_muted: rgba(248,248,232,0.78);
  --primary: #184840;
  --primary2: #104840;
  --accent: #688858;
}
...
.block-container {
  padding-top: 2.2rem;   /* LOWER header area */
...
.header {
  display: flex;
  align-items: flex-end;  /* drop baseline a bit */
  gap: 18px;
  margin-top: 16px;       /* LOWER logo/title */
  margin-bottom: 10px;
}
.header-logo img {
  max-width: 140px;       /* still smaller, but easier to see */
  width: 100%;
  height: auto;
  display: block;
}
.header-title {
  ...
  font-weight: 800;
  line-height: 1.1;
  padding-bottom: 6px;    /* keeps title from hugging top */
}
...
# -------------------------------------------------
# Header (logo left, page title right)
# -------------------------------------------------
PAGE_TITLES = {
